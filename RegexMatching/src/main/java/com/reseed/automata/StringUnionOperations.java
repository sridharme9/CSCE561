...
final public class StringUnionOperations {
	 ...
	 */
	private static com.reseed.automata.State convert(State s, 
			IdentityHashMap<State, com.reseed.automata.State> visited) {
		com.reseed.automata.State converted = visited.get(s);
		if (converted != null)
			return converted;

		converted = new com.reseed.automata.State();
	 ...
	 */
	public static com.reseed.automata.State build(CharSequence[] input) {
		final StringUnionOperations builder = new StringUnionOperations(); 

		for (CharSequence chs : input)
			builder.add(chs);

		return convert(builder.complete(), new IdentityHashMap<State, com.reseed.automata.State>());
	}
